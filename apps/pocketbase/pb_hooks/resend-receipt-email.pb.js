...
onRecordUpdateRequest((e) => {
  try {
    // Check if this is a resend request (you can add a custom header or query param check)
    const requestInfo = e.requestInfo;
    const isResendRequest = requestInfo.query && requestInfo.query.resend === "true";
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .resend-notice { background-color: #fff3cd; border: 1px solid #ffc107; padding: 10px; border-radius: 5px; margin-bottom: 20px; color: #856404; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>Receipt Confirmation (Resent)</h1>
            <p>Here is your receipt as requested.</p>
          </div>

          <div class="resend-notice">
            <strong>Note:</strong> This is a resent copy of your receipt. The original receipt was issued on ${formattedDate}.
          </div>
    ...
    const message = new MailerMessage({
      ...
      subject: "Receipt Resent - Receipt ID: " + receiptId,
      html: htmlContent
    });

    // Send the email
    $app.newMailClient().send(message);
    console.log("Receipt email resent successfully to " + userEmail + " for receipt " + receiptId);

  } catch (error) {
    console.log("Error resending receipt email: " + error.message);
  }
